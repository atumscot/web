...
const datasets = {
	...
	lineColours: {
		rnet: {
			...
			quietness: ["#882255", 25, "#CC6677", 50, "#44AA99", 75, "#117733", 101],
			gradient: [
				"#59ee19",
				3,
				"#37a009",
		...
		data_zones: {
			SIMD2020v2_Decile: [
				"#a50026",
				1.1, // #!# This block is basically enums rather than ranges, so current fudge of .1 is to avoid off-by-one errors
				"#d73027",
				2.1,
				"#f46d43",
				3.1,
			...
			broadband: [
				"#fff7ec",
				0.01, // #!# Currently zero is used for voids - data should be changed to use known constant e.g. -9999
				"#fee8c8",
				2,
				"#fdd49e",
				5,
...
function getLayerWidthField() {
	const purpose = document.querySelector(
		'select.updatelayer[data-layer="rnet"][name="purpose"]',
	).value;
	const type = document.querySelector(
		'select.updatelayer[data-layer="rnet"][name="type"]',
	).value;
	const scenario = document.querySelector(
		'select.updatelayer[data-layer="rnet"][name="scenario"]',
	).value;
...
function rnetStyling(
	...
	const colour = document.querySelector(
		'select.updatelayer[data-layer="rnet"][name="colour"]',
	).value;
	createLegend(datasets.legends.rnet[colour], "linecolourlegend");

	// No special handling needed if not visible
	if (
		!document.querySelector('input.showlayer[data-layer="' + layerId + '"]')
			.checked
	) {
		return;
	}
	...
	const sliders = {};
	document
		.querySelectorAll('input.slider[data-layer="rnet"]')
		.forEach((sliderInput) => {
			...
			[
				...
				[
					...
					[
						"+",
						1,
						["^", 2.718, ["-", 2.94, ["*", ["get", layerWidthField], 0.0021]]],
					],
				],
			],
		],
...
function data_zonesStyling(
	...
	const field = document.querySelector(
		'select.updatelayer[data-layer="data_zones"][name="field"]',
	).value;
	const legendColours = datasets.legends.data_zones.hasOwnProperty(field)
		? datasets.legends.data_zones[field]
		: datasets.legends.data_zones["_"];
	createLegend(legendColours, "dzlegend");

	// Get UI state
	const daysymetricMode = document.querySelector(
		'input.updatelayer[data-layer="data_zones"][name="daysymetricmode"]',
	).checked;
	...
	map.setPaintProperty(
		...
		buildingColour || "#9c9898",
	);
	map.setLayoutProperty(
		"dasymetric",
		"visibility",
		buildingColour ? "visible" : "none",
	);
}
...
function getBuildingsColour(settings) {
	...
	if (
		!document.querySelector('input.showlayer[data-layer="data_zones"]').checked
	) {
		const styleName = document.querySelector(
			"#basemapform input:checked",
		).value; // Same as nptUi.getBasemapStyle()
...
function getStyleColumn(layerId, datasets) {
	const style_col_selected = datasets.lineColours.data_zones.hasOwnProperty(
		layerId,
	)
		? layerId
		: "_";
