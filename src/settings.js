...
const settings = {
	...
	tileserverTempLocalOverrides: {
		// Temporarily define any local folder paths where particular layers should come from
		//rnet: 'utilitytrips/',
	...
	// Manual
	manualEditingUrl:
		"https://github.com/nptscot/nptscot.github.io/edit/dev/%id/index.md",

	// Boundaries
	boundariesUrl:
		"https://nptscot.github.io/scheme-sketcher/assets/boundaries-3d573d2e.geojson",
...
function rnetCheckboxProxying() {
	...
	const rnetsimplifiedCheckboxProxy = document.getElementById(
		"rnet-simplifiedcheckboxproxy",
	);
	const rnetCheckbox = document.querySelector(
		'input.showlayer[data-layer="rnet"]',
	);
	const rnetsimplifiedCheckbox = document.querySelector(
	...
	document.addEventListener("@map/initiallayersset", function (event) {
		const layerProxyEnabled =
			rnetCheckbox.checked || rnetsimplifiedCheckbox.checked;
		const simplifiedModeProxyEnabled = rnetsimplifiedCheckbox.checked;
		rnetCheckboxProxy.checked = layerProxyEnabled;
		rnetsimplifiedCheckboxProxy.checked =
			layerProxyEnabled && simplifiedModeProxyEnabled;
